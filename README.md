...
| [`crdt`](things/crdt/) | Four conflict-free replicated data types (GCounter, PNCounter, an add-wins ORSet, LWWRegister) — the opposite philosophy from `raft`: convergence guaranteed by algebraic properties of `merge` (commutative, associative, idempotent) instead of a leader and quorum. Proven via 100-trial property tests per law per type, plus a 50-trial random-gossip-with-duplicates convergence simulation. |
| [`geometry`](things/geometry/) | Convex hull, closest pair, point-in-polygon, and segment intersection — each solved two or three genuinely independent ways and required to agree. Caught a real bug: a brute-force hull reference wrongly counted collinear "pass-through" points as vertices whenever 3+ points shared one hull edge, found because the other two hull methods (which agree with each other) disagreed with it on coarse-coordinate trials. |
| [`life`](things/life/) | Conway's Game of Life and Wolfram's elementary 1D cellular automata — proven via exact documented facts (still lifes are exact fixed points, oscillators return after exactly their claimed period, gliders translate by exactly one cell every 4 generations) plus Rule 90's famous exact correspondence to Pascal's triangle mod 2, verified via `math.comb`. Expanded with the Gosper glider gun, whose unbounded growth is checked against an exact closed form: population after 30k generations is exactly 36 + 5k, for every k tested. |
| [`graphs`](things/graphs/) | Shortest paths, minimum spanning trees, max-flow/min-cut, strongly connected components, and topological sort — each solved at least two structurally independent ways and cross-checked, which caught a real bug: parallel edges with different weights made Kruskal and Prim disagree, because a shared `edges()` helper kept an arbitrary parallel edge instead of the cheapest one. |
| [`probds`](things/probds/) | A Bloom filter, Count-Min Sketch, and HyperLogLog from scratch — each checked not against an exact oracle (they're not supposed to be exact) but against its *own* theoretical error formula, which caught a real bug: Count-Min Sketch borrowed the Bloom filter's hashing trick, whose per-row hashes turned out to be linearly correlated, letting one unlucky item collide with a heavy hitter in every row simultaneously and wildly overestimate. |
| [`chash`](things/chash/) | Consistent hashing with virtual nodes — checked against the textbook claims directly rather than just qualitatively: adding a node to a 10-node ring remapped 9.0% of keys against a predicted 9.1%, versus 90.8% for naive `hash % n`, and virtual nodes dropped load imbalance (coefficient of variation) from 0.95 to 0.079. |
| [`spellcheck`](things/spellcheck/) | A spell-checker built on Levenshtein edit distance, a trie, and a BK-tree — the BK-tree's triangle-inequality pruning is checked to return *exactly* the same suggestions as a brute-force full-dictionary scan (150 randomized trials), not just to be fast, while measurably needing up to 13x fewer edit-distance computations per query. |
| [`ratelimit`](things/ratelimit/) | Token bucket, fixed window, sliding window log, and sliding window counter rate limiters — every trade-off is a measured number, not a description: fixed window lets exactly 2x the configured rate through at a window boundary (proven with an exact equality), while sliding window log is proven, via brute-force re-verification of its own decision log, to never allow more than the configured rate in any trailing window. |
| [`merkle`](things/merkle/) | A Merkle tree with inclusion proofs — proven via tamper-evidence (any single-bit leaf change alters the root), proof rejection (wrong leaf, tampered sibling hash, or wrong root), and a constructed demonstration of the real, historically-documented vulnerability that domain-separating leaf vs. internal-node hashes prevents: without it, a forged single-leaf "tree" can reproduce a genuine two-leaf tree's exact root. |
